...
export const SOUL = {
  ...
  // 節目の跳ね（ローグウィズ流：数字が大きく跳ねる瞬間で「あと少し」を作る）。線形強化に乗算で重ねる。
  milestoneEvery: 10, // 10レベルごとに「超強化」
  milestoneBonus: 0.1, // 到達した10刻み1つにつき ×(1+0.1) を累積
  megaEvery: 100, // 100レベルごとに「超激強化」
  megaBonus: 0.75, // 到達した100刻み1つにつき ×(1+0.75) を累積
  // 撤退の意味づけ（案1・半減）：倒れると まなび/お金/感情の記憶 がこの割合しか持ち帰れない。
  //  撤退なら 1.0（全部持ち帰る）。魂レベル(到達距離ぶん)と結晶(永続)は 死んでも保持。
  deathKeepRatio: 0.5,
};
...
// ---- こころの木（設計書§8 ④プレイヤー成長）----
//  主人公は転生でリセットされるが、プレイヤー＝こころの木は決してリセットされない。
//  通貨「まなび」をノードに投じて永続強化。器(汎用)＋感情4枝＋隠し枝「共感」。
//  ノード効果は save.js getTreeEffects() で集約し、既存システムに配線する。
export const TREE = {
  evoThresholdFloor: 3, // 進化閾値はこれ未満には下げない（聖域）
  // 「まなび」の獲得式（転生時に確定。到達距離・進化達成・最高更新から）
...
export const LAST_BOSS = {
  ...
  atkMult: 1.4,
  color: 0x8a8ab0,
  intro: ["── 空白の王 ──", "「気持ちなんて、ぜんぶ捨てれば楽になれる」", "世界中に わすれものを 生み出している張本人。"],
  defeatLines: ["空白の王は くずれ落ち、失われた色が 世界に もどっていく。", "でも、まだ 終わりじゃない。ここからが、本当の物語だ。"],
};
// 真章のストーリー（解放後に「物語」タブへ追加される）
export const TRUE_CHAPTER = {
  notices: [
    { id: "tc1", title: "本当の物語", body: "空白の王を倒した。でも、人が気持ちを捨てる理由が消えたわけじゃない。今日もどこかで、だれかが つらい気持ちを手放している。君の旅は、ここからが本番だ。" },
    { id: "tc2", title: "始まりの卵", body: "王がいた場所に、四つの卵が残っていた。怒り・悲しみ・勇気・希望──それぞれの気持ちの赤ちゃんが眠っている。やすらぎの街の巣で卵を選ぶと、次の旅の相棒を その気持ちから育てられる。" },
  ],
};
...
export const BOSS = {
  ...
  //  lore＝ボス登場時に流れる一行。「誰が・どんな気持ちを・なぜ捨てて・今どうなってるか」を素直に。
  types: {
    anger: { name: "炎の魔物", icon: "👹", lore: "大事な人を守れなかった男が、\n『もう怒りたくない』と捨てた怒り。ぶつける相手をなくして、あばれ続けている。" },
    sadness: { name: "涙の魔物", icon: "🌧️", lore: "大切な人と別れた人が、\n『もう泣きたくない』と捨てた悲しみ。深い水になって、みんなを飲みこもうとしている。" },
    courage: { name: "雷の魔物", icon: "⚔️", lore: "挑戦をあきらめた人が、\n『どうせ無理だ』と捨てた勇気。行き場をなくして、空にほえている。" },
    hope: { name: "光の魔物", icon: "🌟", lore: "何度も裏切られた人が、\n『もう期待しない』と手放した希望。それでも、だれかに拾われるのを待っている。" },
  },
  // 空白の王の伏線（生涯ボス撃破数がこの数に達した時、一度だけ流れる。脅威が旅の間に育つ）。
  foreshadow: [
    { atKills: 1, id: "vk1", text: "どこかで、声がした。\n「また 気持ちを拾うのか。そんなの、むだなのに」" },
    { atKills: 3, id: "vk2", text: "声が、少し近づいた。\n「拾っても、人はまた捨てる。きみだって、いつか そうなるよ」" },
    { atKills: 6, id: "vk3", text: "「気持ちなんて、ぜんぶ捨てれば楽になれる」\n── そう言って世界中に気持ちを捨てさせているのが、“空白の王”だ。" },
  ],
};
